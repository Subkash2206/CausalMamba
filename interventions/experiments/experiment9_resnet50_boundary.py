"""
Experiment 9: UNet-ResNet50 Boundary Error Analysis (Phase 1).

Replicates Phase-0 Experiment 6 (Protocol B) logic against the
UNet-ResNet50 architecture:

    - Whole-network low-pass intervention (cutoff=0.25) applied at every
      one of the 9 mapped semantic stages.
    - Computes per-region error rates: Boundary (+-5px, +-10px, +-20px),
      Interior (foreground), and Background.
    - Outputs the Boundary-to-Interior error ratio for comparison against
      VM-UNet's 13.6x ratio (Phase 0, Experiment 6).

Usage:
    cd SpectralMamba
    python ..\\interventions\\experiments\\experiment9_resnet50_boundary.py \\
        --output_dir ..\\interventions\\results\\experiment9_resnet50_boundary --seed 42
"""
...
# ---------------------------------------------------------------------------
# Helpers - IDENTICAL to Phase 0 (Exp 6)
# ---------------------------------------------------------------------------
...
# Mapped semantic boundaries (all 9, identical to resnet50_hook_mapping.py)
HOOK_TARGETS = [
...
def main():
    ...
    print('=' * 80)
    print('Experiment 9: UNet-ResNet50 Boundary Error Analysis (cutoff=0.25)')
    print('=' * 80)
    ...
    # ==================================================================
    # Boundary error computation - IDENTICAL to Phase 0 (Exp 6 Protocol B)
    # ==================================================================
    ...
    _out_dir = _args.output_dir if _args.output_dir else os.path.join(
        os.getcwd(), 'results', 'experiment9_resnet50_boundary')
    ...
    results = {
        ...
        'metadata': {
            'experiment': 'Experiment 9: UNet-ResNet50 Boundary Error Analysis',
            'model': 'UNet-ResNet50 (smp)', 'dataset': 'ISIC2018',
            'num_images': n_images, 'device': str(device), 'seed': _args.seed,
            'timestamp': _ts,
        }
    }
    ...
    csv_path = os.path.join(_out_dir, f'boundary_summary_{_ts}.csv')
