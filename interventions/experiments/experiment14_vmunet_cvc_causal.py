"""
Experiment 14: VM-UNet Whole-Network Causal Intervention on CVC-ClinicDB.

Completes the 2×2 grid (VM-UNet/ResNet50 × ISIC/CVC). Uses the same
FFT→Mask→IFFT pipeline as Experiment 2 (VM-UNet ISIC) but with the
CVCDataset pipeline from Experiment 10 (ResNet50 CVC).
...
Usage:
    cd SpectralMamba
    python ..\\interventions\\experiments\\experiment14_vmunet_cvc_causal.py ^
        --output_dir ..\\interventions\\results\\experiment14_vmunet_cvc_causal --seed 42
"""
...
def main():
    ...
    print('=' * 80)
    print('Experiment 14: VM-UNet CVC-ClinicDB Whole-Network Causal (cutoff=0.25)')
    print('=' * 80)
    ...
    print(f'  Images: {len(val_ds)} | 256x256, [0,1] normalization')
    ...
    out_dir = args.output_dir if args.output_dir else os.path.join(
        os.path.dirname(__file__), '..', 'results', 'experiment14_vmunet_cvc_causal')
    os.makedirs(out_dir, exist_ok=True)

    meta = {
        'experiment': 'Experiment 14: VM-UNet CVC-ClinicDB Whole-Network Causal',
        'model': 'VM-UNet (30-block)', 'dataset': 'CVC-ClinicDB',
        'checkpoint': ckpt_path, 'intervention_type': 'lowpass',
        'cutoff': 0.25, 'img_size': 256, 'num_images': len(val_ds),
